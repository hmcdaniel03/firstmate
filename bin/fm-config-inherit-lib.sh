...
# profile rules, primary config/crew-mcp.json gives a secondmate's own crewmates
# the same firstmate-owned MCP server definitions and default allowlist,
# primary config/crew-harness=codex makes a secondmate's crewmates
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-mcp.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context}"
