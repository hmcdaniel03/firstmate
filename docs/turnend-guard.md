...
`bin/fm-watch-arm.sh --restart` decides whether a recorded lock is its own stale one through that same equivalence, so ownership and stale-lock recovery cannot disagree about which home and watcher script a lock names.
