...
So the same install reached through a mixed-case or symlinked spelling of its home or of `bin/fm-watch.sh` stays healthy, while a genuinely different object, a recorded spelling since repointed at a different home or script, and a recorded home or script that is now missing or unreadable all fail closed and read as no watcher; paths are never lowercased, so case-distinct real objects on a case-sensitive filesystem stay distinct.
