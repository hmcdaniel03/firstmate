...
That check compares the lock's recorded home and watcher script against the current ones as filesystem objects rather than as strings, through `fm_paths_equivalent` in the same library: both sides must still exist with the required type (a directory for the home, a regular file for the script), an identical spelling is then accepted directly, and any other spelling must resolve to the same device and inode.
A recorded relative spelling carries no working directory of its own, so it is resolved by ordinary filesystem lookup in the checking process's current directory against the fixed expected target: it is accepted from a directory where it names the watcher's real home or real watcher script, and fails closed from one where the same spelling names a different real object.
So the same install reached through a mixed-case or symlinked spelling of its home or of `bin/fm-watch.sh` stays healthy, while a genuinely different object, a recorded spelling since repointed at a different home or script, and a recorded home or script that no longer resolves to an existing object of the required type all fail closed and read as no watcher; paths are never lowercased, so case-distinct real objects on a case-sensitive filesystem stay distinct.
`bin/fm-watch-arm.sh --restart` decides whether a recorded lock is its own stale one through that same equivalence, so ownership and stale-lock recovery cannot disagree about which home and watcher script a lock names, but the conservative outcome inverts there: a lock it cannot prove equivalent is not its own stale one, so that lock's pid is never signalled, the lock stays held, and no downtime is published, which is also how a recorded watcher script momentarily absent during a partially applied install is treated.
